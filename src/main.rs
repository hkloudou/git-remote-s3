...
async fn fetch_from_s3(s3: &S3Client, settings: &Settings, r: &GitRef) -> Result<()> {
...
async fn push_to_s3(s3: &S3Client, settings: &Settings, r: &GitRef) -> Result<()> {
    let tmp_dir = Builder::new()
        .prefix("s3_push")
        .tempdir()
        .chain_err(|| "mktemp dir failed")?;
...
async fn cmd_fetch(s3: &S3Client, settings: &Settings, sha: &str, name: &str) -> Result<()> {
    if name == "HEAD" {
...
async fn cmd_push(s3: &S3Client, settings: &Settings, push_ref: &str) -> Result<()> {
...
async fn cmd_loop(s3: &S3Client, settings: &Settings) -> Result<()> {
    loop {
...
async fn cmd_list(s3: &S3Client, settings: &Settings) -> Result<()> {
